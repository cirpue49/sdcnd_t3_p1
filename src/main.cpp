...
//    vector<double> horizon = {0.5, 1.0, 1.5};
    ...
    {
      double plus_s = check_speed * horizon[j];
      check_car_s += plus_s;
      agent_s.push_back(check_car_s);
    }
    predictions_for_agents.push_back(agent_s);
  }
  return predictions_for_agents;
}

vector<vector<double>> filter_sensor_fusion(vector<vector<double>> sensor_fusion, int lane){
  vector<vector<double>> filtered;
  for(int i = 0; i < sensor_fusion.size(); i++)
  {
    float d = sensor_fusion[i][6];
    //just one loop if len(lanes) = 1
    if( d < (2+4*lane+2) && d > 2+4*lane-2)
    {
      filtered.push_back(sensor_fusion[i]);
    }
  }
...
bool safe_lane_changing(double ego_s , vector<vector<double>>predictions){
  bool safe = true;
          ...
          // j[1] is the data JSON object
            ...
            //size of sensor fusion: 12
          	...
          	vector<double> next_y_vals;
						...
						json msgJson;
          	// car will visit sequentially every .02 seconds
